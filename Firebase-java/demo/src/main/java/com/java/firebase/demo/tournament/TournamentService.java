...
public class TournamentService {
    ...
    public String createPlayer(String tournamentName, String participatingPlayerName)
        ...
        // ApiFuture<WriteResult> updateResult = playerDocRef.update("pastMatches",
    ...
    public String updatePlayerMatch(String tournamentName, String participatingPlayerName, String matchId)
            throws InterruptedException, ExecutionException {
    ...
    public ParticipatingPlayer getPlayer(String tournamentName, String participatingPlayerName)
            throws InterruptedException, ExecutionException {
        ...
        // Gary: I recommend this change, so the problem fails early before the frontend
        // starts wondering
        ...
        // Check if the document exists
        // if (!document.exists()) {
        // throw new IllegalArgumentException("The tournament or player does not
        // exists.");
        // }
        ...
        // System.out.println("Player document found");
        // ParticipatingPlayer participatingPlayer =
        // document.toObject(ParticipatingPlayer.class); // Can be converted to JSON if
        // required
    ...
    public List<Match> getPlayerPastMatches(String tournamentName, String participatingPlayerName)
            throws InterruptedException, ExecutionException {
        ...
        for (String matchId : pastMatchIds) {
            ...
            Match match = getMatch(tournamentName, matchMap.get("roundName"), matchMap.get("player1"),
                    matchMap.get("player2"));
            pastMatches.add(match);
        }
        return pastMatches;
    }

    public String deletePlayer(String tournamentName, String participatingPlayerName)
            throws InterruptedException, ExecutionException {
        try {
            // Step 1: Delete the player document from the "participatingPlayer"
            // subcollection
            ...
            // Step 2: Remove the player from the "participatingPlayers" array in the
            // tournament document
            DocumentReference docRef = firestore.collection("tournament").document(tournamentName);

            ApiFuture<WriteResult> updateFuture = docRef.update("participatingPlayers",
                    FieldValue.arrayRemove(participatingPlayerName));
    ...
    // This takes one of the specified json fields, here .getTournamentName(), and
    // sets it as the documentId (document identifier)
    ...
    // The key in the GET request must be "documentid", and the value is the
    // tournamentName
    ...
    public List<Round> fetchRounds(String tournamentName) throws ExecutionException, InterruptedException {
        ...
        QuerySnapshot roundsSnapshot = firestore.collection("tournament").document(tournamentName).collection("round")
                .get().get();

        // DocumentReference documentReference =
        // firestore.collection("tournament").document(tournamentName);
    ...
    public List<String> fetchParticipatingPlayers(String tournamentName)
    ...
    /**
     * Retrieves a list of all tournaments in which a specific player is
     * participating.
     *
     * @param playerName the name of the player whose tournaments are to be
     *                   retrieved.
     * @return a list of tournament names where the player is participating.
    ...
    public List<String> getAllTournamentForPlayer(String playerName) throws ExecutionException, InterruptedException {
        ...
        List<String> allTournamentNames = getAllTournamentNames(); // Retrieve all tournament names.

        for (String tournament : allTournamentNames) {
            List<String> tournamentPlayers = getAllPlayer(tournament); // Get all players in the tournament.
            for (String player : tournamentPlayers) {
                if (player.equals(playerName)) { // Check if the player is part of the tournament.
                    result.add(tournament);
                    continue;
                }
            }
        }
        return result; // Return the list of tournaments the player is in.
    }

    /**
     * Updates an existing tournament in Firestore.
     *
     * @param tournament the tournament object containing updated information.
     * @return a success or failure message based on whether the update was
     *         performed.
     * @throws ExecutionException   if an error occurs during Firestore operations.
     * @throws InterruptedException if the operation is interrupted.
     */
    public String updateTournament(Tournament tournament) throws ExecutionException, InterruptedException {
        // Validate the tournament object. An error is thrown if invalid.
        if (tournamentValidator.isTournamentValid(tournament)) {
        ...
        if (document.exists()) {
            // Update the document with the new values, merging only changed fields.
            ApiFuture<WriteResult> collectionsApiFuture = tournamentDocRef.set(tournament, SetOptions.merge());
    ...
    /**
     * Checks if a tournament with the specified name exists in Firestore.
     *
     * @param tournamentName the name of the tournament to check.
     * @return true if the tournament exists, false otherwise.
     * @throws ExecutionException   if an error occurs during Firestore operations.
     * @throws InterruptedException if the operation is interrupted.
     */
    ...
    /**
     * Deletes a tournament and all its associated data from Firestore.
     *
     * @param tournamentName the name of the tournament to delete.
     * @return a success or failure message based on whether the delete was
     *         performed.
     * @throws ExecutionException   if an error occurs during Firestore operations.
     * @throws InterruptedException if the operation is interrupted.
     */
    public String deleteTournament(String tournamentName) throws ExecutionException, InterruptedException {
        ...
        for (DocumentSnapshot playerDoc : playersSnapshot.get().getDocuments()) {
            ...
            deletePlayer(tournamentName, playerName); // Delete individual players.
        }
        ...
        for (DocumentSnapshot roundDoc : roundsSnapshot.get().getDocuments()) {
            ...
            // Delete each match in the round.
            ...
            // Delete each standing in the round.
    ...
    public String createRound(String tournamentName, Round round) throws ExecutionException, InterruptedException {
        ...
        // iteratively create matches and to firestore. A round may or may not contain
        // matches
    ...
    public String updateRound(String tournamentName, Round round) throws ExecutionException, InterruptedException {
        ...
        if (document.exists()) {
            ...
            ApiFuture<WriteResult> collectionsApiFuture = documentReference.set(round, SetOptions.merge()); // Update
                                                                                                            // only new
                                                                                                            // values
    ...
    // now for matches, winner wins losses isDraw isBye can be empty for now, but
    ...
    public String roundEnd(String tournamentName, String roundName) throws ExecutionException, InterruptedException {
        ...
        // processRoundData(tournamentName, round); //based on the matches in the round,
        // go and update FB player's matches with the ID
        if (!isTournamentInProgress(tournamentName)) {
    ...
    public String createMatch(String tournamentName, String roundName, Match match)
            throws ExecutionException, InterruptedException {
        ...
        return "One match created in " + tournamentName + ", " + roundName + " at "
                + matchResult.get().getUpdateTime().toString();
    }

    public Match getMatch(String tournamentName, String roundName, String player1, String player2)
            throws ExecutionException, InterruptedException {
    ...
    public String updateMatch(String tournamentName, String roundName, String player1, String player2,
            Match updatedMatch) throws ExecutionException, InterruptedException {
        ...
        if (document.exists()) {
            // Document exists, proceed with the update
            ApiFuture<WriteResult> matchResult = matchDocRef.set(updatedMatch, SetOptions.merge()); // Update only
                                                                                                    // provided fields
            matchResult.get(); // Wait for completion
    ...
    public String deleteMatch(String tournamentName, String roundName, String player1, String player2)
            throws ExecutionException, InterruptedException {
        // Generate the documentId based on tournament, round, and player names
        String documentId = generateMatchId(tournamentName, roundName, player1, player2);
    ...
    public String createStanding(String tournamentName, String roundName, Standing standing)
            throws ExecutionException, InterruptedException {
        // String documentId = String.valueOf(standing.getRank()); // Using rank as the
        // document ID
    ...
    public Standing getStanding(String tournamentName, String roundName, int rank)
            throws ExecutionException, InterruptedException {
        ...
        if (!documents.isEmpty()) {
            ...
            throw new NoSuchElementException("Standing with rank " + rank + " not found in round " + roundName
                    + " of tournament " + tournamentName);
        }
    }

    public List<Standing> getAllStanding(String tournamentName, String roundName)
            throws ExecutionException, InterruptedException {
        // Retrieve all documents from the "tournament" collection
        ApiFuture<QuerySnapshot> future = firestore.collection("tournament")
                .document(tournamentName)
    ...
    public String updateStanding(String tournamentName, String roundName, Standing updatedStanding)
            throws ExecutionException, InterruptedException {
        ...
        // Update the standing document only for the fields that are provided (merge
        // operation)
        ApiFuture<WriteResult> standingResult = standingDocRef.set(updatedStanding, SetOptions.merge()); // Only update
                                                                                                         // the provided
                                                                                                         // fields
        standingResult.get(); // Wait for completion
    ...
    public String deleteStanding(String tournamentName, String roundName, int rank)
            throws ExecutionException, InterruptedException {
    ...
    public boolean generateRound(String tournament) throws ExecutionException, InterruptedException {
        ...
        if (tourney != null) {
            ...
            // input all participating players into algoObjs and put them into the list
            // algoPlayers
            ...
            List<AlgoTournamentPlayer> algoPlayers = parsePlayersIntoAlgoObjects(tourney, playerToPastMatches,
                    playerIDToObj);
    ...
    public Match getMatch(String tournamentName, String roundName, String documentId)
            throws ExecutionException, InterruptedException {
        System.out.println("getMatch starting");
        // Generate the same documentId used in createMatch
        System.out.println("DocumentId to be searched: " + documentId);
    ...
    public Match getMatchByPlayer(String tournamentName, String roundName, String player)
            throws ExecutionException, InterruptedException {
    ...
    /*
     * All helper functions for parsing the AlgoObjects to DB objects and DB
     * creation and updating all below
    ...
    public void updateDataBaseWithMatches(Tournament tournament, String roundName, List<Match> matchesToupdate)
            throws ExecutionException, InterruptedException {
        ...
        for (Match match : matchesToupdate) {
            ...
            if (!match.isBye()) {
                ...
                updatePlayerMatch(tournament.getTournamentName(), player2Name,
                        generateMatchId(tournament.getTournamentName(), roundName, match));

            }

            updatePlayerMatch(tournament.getTournamentName(), player1Name,
                    generateMatchId(tournament.getTournamentName(), roundName, match));
    ...
    public List<AlgoTournamentPlayer> parsePlayersIntoAlgoObjects(Tournament tournament,
            HashMap<AlgoTournamentPlayer, List<String>> playerToPastMatches,
            HashMap<String, AlgoTournamentPlayer> playerIDToObj) throws ExecutionException, InterruptedException {

        // Differs from parseIntoAlgoRd1 as there is the need to map past match record
        // to player objects
    ...
    public void parseMatchHistoryIntoPlayerObjs(List<AlgoTournamentPlayer> algoPlayers, Tournament tournament,
            HashMap<AlgoTournamentPlayer, List<String>> playerToPastMatches,
            HashMap<String, AlgoTournamentPlayer> playerIDToObj) throws ExecutionException, InterruptedException {
    ...
    public void updateStandingsinDB(AlgoStandings algoStandings, String tournament, Tournament tourney)
            throws ExecutionException, InterruptedException {
